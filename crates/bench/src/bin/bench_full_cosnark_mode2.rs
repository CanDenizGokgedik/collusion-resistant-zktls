//! Full Π_coll-min pipeline benchmark with real co-SNARK Mode 2 (full TLS-PRF).
//!
//! RC Phase → dx-DCTLS (Groth16 Mode 2, ~1.9M R1CS) → FROST Sign → On-Chain ABI
//!
//! CRS is set up once and reused across all configurations.
//! Note: Mode 2 prove ≈ 21s per attestation on Apple M1.
//!
//! # Usage
//! ```bash
...
use tls_attestation_zk::co_snark::CoSnarkBackend;
use tls_attestation_zk::tls_prf_circuit::pms_hash;
use tls_attestation_zk::{MacKey, PreMasterSecret, split_mac_key, derive_k_mac_from_pms};
use tls_attestation_core::{hash::DigestBytes, ids::{SessionId, VerifierId}};

// ── Fixed TLS session params (same across all configs) ────────────────────────

const PMS:           [u8; 48] = [0x22u8; 48];
const CLIENT_RANDOM: [u8; 32] = [0x01u8; 32];
const SERVER_RANDOM: [u8; 32] = [0x02u8; 32];

// ── Mode 2 co-SNARK executor ──────────────────────────────────────────────────
...
impl CoSnarkExecutor for Mode2CoSnark {
    fn execute(
        &self,
        _p_share:     &[u8; 32],
        _v_share:     &[u8; 32],
        ...
        use tls_attestation_zk::tls12_hmac::{
            ProverMacKeyShare, VerifierMacKeyShare, combine_mac_key_shares,
        };

        let p = ProverMacKeyShare(self.p_share);
        let v = VerifierMacKeyShare(self.v_share);
        let k_mac = combine_mac_key_shares(&p, &v);

        let proof = self.backend
            .execute_mode2(&p, &v, rand_binding, &PMS, &CLIENT_RANDOM, &SERVER_RANDOM)
...
fn run_pipeline(
    ...
    // Attestation Phase (Mode 2 Groth16)
    let t1 = Instant::now();
    ...
    let t3 = Instant::now();
    let mut sig_rx = [0u8; 32]; let mut sig_s = [0u8; 32];
    let mut gk_x   = [0u8; 32]; let mut gk_y  = [0u8; 32];
    ...
    let att = OnChainAttestationSecp256k1 {
        ...
        group_key_x: gk_x, group_key_y: gk_y,
        sig_R_x: sig_rx, sig_s,
        threshold: threshold as u8, verifier_count: n_verifiers as u8,
        alpha_commitment: [0x42u8; 32], session_id: [0x00u8; 32],
    };
...
fn main() {
    println!("╔══════════════════════════════════════════════════════════════════╗");
    println!("║  Π_coll-min Full Pipeline — Real co-SNARK (Groth16 Mode 2)      ║");
    println!("║  Full TLS-PRF circuit: ~1.9M R1CS constraints (BN254/arkworks)  ║");
    println!("╚══════════════════════════════════════════════════════════════════╝\n");

    // Derive consistent K_MAC from PMS.
    let pms_typed = PreMasterSecret(PMS);
    let k_mac = derive_k_mac_from_pms(&pms_typed, &CLIENT_RANDOM, &SERVER_RANDOM);
    let (p_share, v_share) = split_mac_key(&k_mac, &mut OsRng);

    // One-time CRS setup.
    print!("  Groth16 CRS setup (Mode 2, ~1.9M R1CS)... ");
    std::io::Write::flush(&mut std::io::stdout()).ok();
    let t = Instant::now();
    let backend = CoSnarkBackend::setup_mode2().expect("mode2 CRS setup");
    println!("{}ms", t.elapsed().as_millis());

    let cosnark = Mode2CoSnark { backend, p_share: p_share.0, v_share: v_share.0 };

    // Fewer configs — each Attest takes ~21s.
    let configs: &[(usize, usize)] = &[
        (3, 5), (5, 9), (7, 13), (10, 19), (15, 29), (20, 39), (30, 59), (50, 99),
    ];
    ...
    println!();
    println!("  Notes:");
    println!("  • Attest = Groth16 Mode 2 (~1.9M R1CS, BN254). Paper: 4,700ms (gnark/BLS12-381).");
    println!("  • RC dominates at large n due to O(n²) DKG; Attest stays constant (O(1)).");
    println!("  • Sign = FROST in-process (no network latency).");

    let json = serde_json::json!({
        "benchmark": "full-pipeline-cosnark-mode2",
        "co_snark_mode": "mode2",
        "r1cs_constraints": 1_927_271u32,
        "results": results.iter().map(|&(t, n, rc, attest, sign, onchain, total)| {
            serde_json::json!({
                "config": format!("{}-of-{}", t, n),
                "rc_ms": rc, "attest_ms": attest,
                "sign_ms": sign, "onchain_ms": onchain, "total_ms": total,
            })
        }).collect::<Vec<_>>(),
    });
